...
class Table:
    ...
    def render(self):
        ...
        for i in range(min(5, len(self.top))):
            ...
            text = pygame.transform.scale(font.render(str(i + 1), True, TEXT_COLOR),
                                          (self.ratio[0] * self.width, self.height))
            self.screen.blit(text, (self.x + sum(self.ratio[:0]) * self.width + 2, self.y + i * self.height))
            pygame.draw.rect(self.screen, BLACK,
            ...
            text = pygame.transform.scale(
                font.render(str(self.top[i][1]).rjust(5, '0'), True, TEXT_COLOR),
                (self.ratio[2] * self.width, self.height))
            self.screen.blit(text, (self.x + sum(self.ratio[:2]) * self.width, self.y + i * self.height))
